...
函数功能：所有的控制代码都在这里面
         ...
         严格保证采样和数据处理的时间同步				 
**************************************************************************/
int EXTI15_10_IRQHandler(void)    //在启动文件夹里面
{    
	 if(INT==0)		
	 {    
				  ...
				  if(CAN_ON_Flag==0&&Usart_ON_Flag==0&&PS2_ON_Flag==0)  
					{  
						...
						if(judge >=1 )
						{
							if(PDin(2) == 1 && flag == 0 )
							{
								Kinematic_Analysis(0,RC_Velocity,0);
								flag = 1;
							}
						...
						if(flag ==0 ) Get_RC(Run_Flag);
						
					}                //===串口和CAN控制都未使能，则接收蓝牙遥控指
...
{
	    int siqu=0;
    	if(motor_a>0)			PWMA=motor_a+siqu, INA=0;     //INA连接着PCout(4) 
		  ...
		  if(CAN_ON_Flag==1||Usart_ON_Flag==1) 
				{
					 if(rxbuf[0]==1)
					 {
						 ...
						 Move_X=flag_1*(rxbuf[1]*256+rxbuf[2]);     //X轴的目标速度
					 ...
					 if(rxbuf[0]==2)
					 {
						 ...
						 Target_A=flag_1*rxbuf[1];     //电机的目标值
